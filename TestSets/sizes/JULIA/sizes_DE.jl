...
#@objective(model, Min, sum(sizes.P[i]*y[i,t] for i in sizes.N for t in sizes.T)
#                        + sum(sizes.Pr[l]*(sum((sum(sizes.s*z[i,t,l] for i in sizes.N) + sizes.r*sum(x[i,j,t,l] for i in sizes.N[2:end] for j in 1:i-1)) for t in sizes.T)) for l in sizes.L))

@objective(model, Min, sum(sizes.s*z1[i] + sizes.P[i]*y1[i] for i in sizes.N) + sizes.r*sum(x1[i,j] for i in sizes.N[2:end] for j in 1:i-1)
...
for i in sizes.N, t in sizes.T, l in sizes.L
    if getvalue(z[i,t,l]) >= 1
        println("z[$i,$t,$l] = $(getvalue(z[i,t,l]))")
    end
end
