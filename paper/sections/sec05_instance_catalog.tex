...
Let $Y_R$ be the continuous relaxation of $Y$. $z_{LP2}$ is defined by the objective value of the following partly LP-relaxed problem:
...
\begin{align}
  z_{EEV} := c^T \bar{x} + \mathcal{Q}(\bar{x})
\end{align}
...
where $z^*(x,y_s)$ is the optimal objective value of a \textit{single scenario problem} given a scenario $s$: 
	...
	\begin{align}
	z_{SS(s)}:= \min_x\ &\ \left\{ c^T x + Q(x,\xi_s) : Ax \ge b, \; x\in X \right\}
	\end{align}
	
\begin{subequations}
	\begin{align}
	z_{SS(s)}:= \min_{x,y_s}\ &\ c^\top x+q_s^\top y_s \label{ssp:obj}\\
