...
We use two solvers, a general purpose commercial MIP solver \cplex\ 12.8 and an open-source DD-based SIP solver \dsp. For solving master/sub problems during DD procedure in \dsp, we also use \cplex\ 12.8. Since \cplex\ does not support \smps\ as its input format, we use \dsp's capability to solve the EF problem by reading \smps\ files. We fully utilize 36 threads for each single instance as the both two solvers support multi-threads parallel computing. We set the termination condition on CPU time by 3 hours. 
...
%		\begin{tabular}{@{}ll@{}}
		\begin{tabular}{@{}ll@{}}
			...
			$z_{WS}$ & Expected objective value of single scenario problem (\ref{ssp:obj})-(\ref{ssp:e})\\
			\bottomrule
		\end{tabular}%
