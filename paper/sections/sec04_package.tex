...
\subsection{Adding SIP problem into the package}
Adding new problem to the package is as simple as adding a line to the existing file and implementing one or two scripts for the problem. The modification and implementation should follow the predefined way, e.g., folder name, script name, composite-type name, function name, and arguments of the function.
...
The modeling function for each problem is defined in the script \textit{model}. When an user wants to add a new problem, this script is the minimum required implementation. Sometimes the data generating function is separated from \textit{model} by \textit{data} when the data generating procedure is so complicated that makes the modeling function too long (e.g., \suc).
\begin{quote}
	\noindent\textit{model} This kind of script contains a definition of the function that constructs and returns \jumpmodel-type object of an instance. This function calls \jump\ and \structjump\ packages. When there is a separated \textit{data} script, it must be included in this script. Fig \ref{fig:DCAP_model.jl} shows the example of \dcap: \texttt{DCAP\_model.jl}
\end{quote}
\begin{quote}
