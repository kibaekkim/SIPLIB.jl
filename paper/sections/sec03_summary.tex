...
%	\resizebox{\textwidth}{!}{%
		\begin{threeparttable}
			...
			\begin{tabular}{@{}llp{3in}}
				...
				\sizes  &  \makecell[tl]{We modify the formulation to polish (\ref{sizes:obj})-(\ref{sizes:h}), We develop data generation \\procedure}                                                 \\
				\smkp   & -                                                                         \\
...
Although no universally effective way to measure the difficulty of MIP is discovered yet, the number of components is definitely one of the closely related factor. For example, instances tend to be more difficult as the number of discrete variables increases. The continuous variables, which hardly are problematic when solely present by themselves, can also let the instance burdensome when they are mixed with integer variables. 
