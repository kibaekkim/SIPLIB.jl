\yoc{I strengthened this section towards describing why the things we consider are important. Important arguments are colored in purple.}
	...
	\resizebox{\textwidth}{!}{%
		\begin{threeparttable}
			...
			\begin{tabular}{@{}cl@{}}
				...
				\mptsps & We adopt formulation from another reference \cite{journal:LSD1990}, We develop data generation procedure \\
				\sizes  & We modify the formulation to polish                                                    \\
				\smkp   & -                                                                         \\
...
\textcolor{purple}{Although no universally effective way to measure the difficulty of MIP is discovered yet, the number of components in instance is one of the closely related factor. For example, instances tend to be more difficult as the number of discrete variables increases. The continuous variables, which hardly are problematic when solely present by themselves, can also let the instance burdensome when mixed with integer variables. } 
...
\kk{$T$ has been called ``technology matrix'' in the literature. Let's not introduce new term, unless we are proposing new concept related to the name.}
\yoc{Thanks. I change the term throughout the paper and package.}
