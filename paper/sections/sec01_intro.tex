...
The optimizers, not only SIP researchers, have always needed test instances to figure out the performances of newly developed solver and algorithm. \texttt{MIPLIB 2010} \cite{MIPLIB} is one good example of the collections of test instances for optimization problems hence the main motif of \siplibtwo. 
...
\begin{itemize}
	\item We provide ready-made SIP test instances more various in terms of problem size and types than the former \siplib.
	\item We provide summarized problem-specific details: mathematical formulation, stochastic data generation.
	\item We provide utilities for generating/pre-analyzing instances.
	\item We provide pre-analysis on the ready-made instances.
	\item We provide systematically summarized benchmarking experimental results.
\end{itemize}
