...
regarding the problems that can be fully implementable based on the open information \kk{What does this mean?}\yoc{Shabbir's SIPLIB provides instances of 6 2-stage SIPs (DCAP, MPTSPs, SEMI, SMKP, SIZES, SSLP) but SEMI does not open the data generation procedure at all. Hence, we have 5 implemantable problems (although some of them are still not exactly the same as original due to the missing information, e.g., MPTSPs) from Shabbir's SIPLIB.}
