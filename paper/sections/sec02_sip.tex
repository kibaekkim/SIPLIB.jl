...
\begin{table}[]
	...
	%			$z\in\mathbb{R}$ & optimal objective value of the SIP \\ 
				...
				$\phi_{\xi_s}:\mathbb{R}^{m_2}\to\mathbb{R}$	&  optimal objective function of the second-stage program given a scenario $\xi_s$	\\
...
To briefly explain the idea of each algorithm, we introduce the following terms for solution systems:
\begin{itemize}
...
The main idea of DD algorithm is to relax the nonanticipativity constraints using Lagrange multipliers and then to restore them. Given set of multipliers, the problem is separable by scenarios hence the associated dual function can be optimized scenario-wise independently \cite{journal:CS1999}. The fundamental steps of the DD procedure can be summarized as follows:
\begin{enumerate}
