...
%\begin{align}
%\textrm{(RP) }\min_{x\in X} z(x):={\left\{c^\top x + \mathcal{Q}(x):\ Ax\ge b\right\}}, \label{eq:rp}
%\end{align}
...
The RP (\ref{eq:rp}) can be then rewritten in the \textit{extensive form} (EF)
...
Assuming finite and discrete distribution for scenarios, we can always formulate SIP in EF as in Equations (\ref{ef:obj})-(\ref{ef:e}).  %much of the solution methods in SIP is studied to resolve the difficulty of optimizing the form in Equation \ref{eq:rp}, 
