...
\begin{subequations} \label{airlift:formulation}
	...
	&-\sum_{i\in A}\sum_{k\in R\backslash\{j\}}b_{ij}\left(\frac{\alpha_{ijk}}{a_{ij}}\right)\chi_{ijk}^s + \sum_{i\in A}\sum_{k\in R\backslash\{j\}}b_{ij}\chi_{ikj}^s + y_j^{+s} - y_j^{-s} \nonumber \\
	&\qquad\qquad\qquad\qquad\qquad\qquad= d_j^s - \sum_{i\in A}b_{ij}x_{ij},\quad \forall j\in R,\ \forall s\in\mathcal{S}, \label{airlift:d}\\
...
%		{@{}llp{3in}}
		\begin{tabular}{ll}
			...
			$c_j^+$ & unit cost of commercially contracted transport on route $j$\\
			$c_j^-$ & unit penalty cost per of unused carrying capacity on route $j$\\
			...
			$y_j^{+s}$ (2nd-stage) & amount of commercial demand on route $j$ under scenario $s$\\
...
For model parameters, we use a reduced model of the CAISO to follow the reference \cite{journal:AF2004}. The model includes a sparse representation of the entire WECC western interconnect outside of California. 
