...
\texttt{MPTSPs} is a variant of the travelling salesman problem (TSP) where a set of paths exists between any two nodes and each path is characterized by a random travel time. 
...
\subsection{\texttt{SIZES}: Selection of an optimal subset of sizes} \label{SIZES}
...
\subsection{\texttt{SSLP}: Stochastic server location problem} \label{SSLP}
...
\begin{subequations} \label{SUC:formulation}
	...
	\label{SUC:obj}\\
	\textrm{s.t.}\ &  \sum_{q=t-UT_g+1}^t z_{gq}\le w_{qt},\quad\forall g\in G_s,\ \forall t\ge UT_g,	\label{SUC:b}\\
...
\begin{table}[H]
	\centering
	\caption{Notations for the \texttt{SUC}}
	\label{SUC:notation}
	\begin{tabular}{ll}
		\toprule
		\multicolumn{2}{l}{\textbf{Index sets:}} \\
		...
		\multicolumn{2}{l}{\textbf{Decision variables:}} \\
		$w_{gt}$ (1$^{\textrm{st}}$ stage) 	& (binary) commitment of slow generator $g$ at time $t$\\
		$z_{gt}$ (1$^{\textrm{st}}$ stage)	&	(continuous) start-up of slow generator $g$ at time $t$\\
		$u_{gts}$ (2$^{\textrm{nd}}$ stage)	&	(binary) commitment of fast generator $g$ at time $t$ under scenario $s$\\
		$v_{gts}$ (2$^{\textrm{nd}}$ stage)	&	(continuous) startup of fast generator $g$ at time $t$ under scenario $s$\\
		$\theta_{gts}$ (2$^{\textrm{nd}}$ stage)	&	(continuous) phase angle of generator $g$ at time $t$ under scenario $s$\\
		$e_{lts}$ (2$^{\textrm{nd}}$ stage)	&	(continuous) power flow on line $l$ at time $t$ under scenario $s$\\
		$p_{gts}$ (2$^{\textrm{nd}}$ stage)	&	(continuous) production of generator $g$ at time $t$ under scenario $s$\\
		$x_{\lambda ts}^\mathcal{L}$	(2$^{\textrm{nd}}$ stage) & (continuous) load shedding for load $\lambda$ at time $t$ under scenario $s$	\\
		$x_{\iota ts}^\mathcal{I}$	(2$^{\textrm{nd}}$ stage) &	(continuous) spillage for import point $\iota$ at time $t$ under scenario $s$\\
		$x_{\rho ts}^\mathcal{R}$ (2$^{\textrm{nd}}$ stage)	& (continuous) spillage for non-wind renewable generator $\rho$ at time $t$ under scenario $s$  	\\
		$x_{\omega ts}^\mathcal{W}$ (2$^{\textrm{nd}}$ stage)	& (continuous) spillage for wind generator $\omega$ at time $t$ under scenario $s$ 	\\
		\hline
	\end{tabular}
\end{table}

\subsubsection{\texttt{SUC}: Data generation}
...
Based on the WECC, set cardinalities in \texttt{SUC} are as follows.
\begin{align*}
|N|&=225 \\
|T|&=24 \\
|L|&=375 \\
...
The maginal cost parameters for additional sources (load, import, non-wind renewable, wind) are fixed by
\begin{align*}
C^\mathcal{L}&=5000,\\
...
Unlike other problems where stochastic data is generated while \texttt{Julia} script is running, we pre-generate and provide up to 1,000 wind power production profiles for each day type. This data is included in ``/SUC/DATA'' folder and used to generate the stochastic parameter $IC_{\omega ts}^\mathcal{W}$. Hence, the number of scenarios that can be considered in an instance is limited to 1,000, which we think is large enough regarding the intrinsic large-scale size of the problem \texttt{SUC}.
