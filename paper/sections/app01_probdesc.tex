...
\begin{subequations} \label{sucw:formulation}
	\begin{align}
	(\texttt{SUCW})\ \textrm{min}\ &\sum_{g\in G_s}\sum_{t \in T}\left(K_g w_{gt}+S_g z_{gt}\right) \\	&+ \sum_{s\in\mathcal{S}}\PP(s)\sum_{t\in T} 
	\Bigg[	 \sum_{g\in G_f} \left(K_g u_{gt}+S_g v_{gt}\right) + \sum_{g\in G}C_g p_{gt} + \sum_{j\in J} C^{J}\lambda_{dt} \\&\qquad\qquad\qquad\qquad  + \sum_{i\in I} C^{I}\mu_{it} + \sum_{r\in R} C^{R}\lambda_{rt} + \sum_{k\in W} C^{W}\omega_{kt} \Bigg]
	\label{smkp:obj}\\
	\textrm{s.t.}\ &  \sum_{i\in I}a_{ji}x_{i} + \sum_{i \in I}e_{ji}z_i\ge b_j,\quad\forall j\in J, \label{smkp:b}\\
	\end{align}	
\end{subequations}
...
\begin{table}[H]
	...
	\begin{tabular}{ll}
		...
		$T$ & index set of time periods ($t\in T$)\\
		$J$ & index set of loads for shedding ($j\in J$)\\
		$I$ & index set of import points ($i\in I$)\\
		$W$ & index set of wind power generators ($k\in W$)\\
		$R$ & index set of non-wind renewable generators ($r\in R$)\\
		$L_n^{+}$ & index set of outgoing transmission lines from bus $n$ \\
		$L_n^{-}$ & index set of incoming transmission lines to bus $n$ \\
		$\mathcal{S}$ & index set of scenarios ($s\in\mathcal{S}$) 		\\ \midrule
		\multicolumn{2}{l}{\textbf{Parameters:}} \\
		...
		$C^J$ & marginal load shedding cost of loads in $J$	\\
		$C^I$ & marginal spillage cost of import points in $I$ \\
		$C^R$ & marginal spillage cost of renewable generators in $R$ \\ 
		$C^W$ & marginal spillage cost of wind generators in $W$ \\
		\multicolumn{2}{l}{\textbf{(capacity)}} \\
		$B_{ls}$ & susceptance of line $l$ under scenario $s$\\
		$P_{gs}^+$ & maximum generation capacity of generator $g$ under scenario $s$\\
		$P_{gs}^-$ & minimum generation capacity of generator $g$ under scenario $s$\\
		$R_{gs}^+$ & maximum ramping capacity of generator $g$ under scenario $s$\\
		$R_{gs}^-$ & minimum ramping capacity of generator $g$ under scenario $s$\\
		...
		$D_{nts}$ & net demand in bus $n$ at time $t$ under scenario $s$\\
		$IC_i^I$ & generation from import point $i$\\
		$IC_r^R$ & generation from renewable generator $r$\\
		$IC_k^W$ & generation from wind generator $k$\\
		$IC_j^J$ & shedding from load $j$\\
		\multicolumn{2}{l}{\textbf{(mapping to bus)}} \\
		$m^{G}(g)$ & the index of the bus connected by generator $g$ \\
		$m^{I}(i)$ & the index of the bus connected by import point $i$ \\
		$m^{J}(j)$ & the index of the bus that can shed load $j$ \\
		$m^{R}(r)$ & the index of the bus connected by renewable generator $r$ \\
		$m^{W}(k)$ & the index of the bus connected by wind generator $k$ \\ \midrule
		\multicolumn{2}{l}{\textbf{Decision variables:}} \\
		...
		$\lambda_{jts}$	(2$^{\textrm{nd}}$ stage) & load shedding for load $j$ at time $t$ under scenario $s$	\\
		$\mu_{its}$	(2$^{\textrm{nd}}$ stage) &	spillage for import point $i$ at time $t$ under scenario $s$\\
		$\rho_{rts}$ (2$^{\textrm{nd}}$ stage)	& spillage for non-wind renewable generator $r$ at time $t$ under scenario $s$  	\\
		$\omega_{kts}$ (2$^{\textrm{nd}}$ stage)	& spillage for wind generator $k$ at time $t$ under scenario $s$ 	\\
		\hline
	\end{tabular}
\end{table}
