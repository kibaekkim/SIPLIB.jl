...
\begin{subequations}
\begin{align}
z=\min_{x,y_j}\ &c^{\top}x + \sum_{j=1}^{r}p_j (q_j^{\top}y_j), \\ 
\mathrm{s.t.}\ &Ax\ge b,  \\
	&T_j x+W_j y_j\ge h_j,\quad\forall j\in\{1,\ldots,r\},\\
	&x\in X, \\
	&y_j \in Y,\quad\forall j\in\{1,\ldots,r\}.
\end{align}
\end{subequations}
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}
	{
		\begin{tabular}{ll}
			...
			$Y\subseteq\mathbb{R}^{n_2}$	& second-stage polyhedral set (continuous, integer, binary)\\ \midrule
			...
			$y_j\in\mathbb{R}^{n_2}$	& second-stage decision vector under scenario $\xi_j$	\\
			$q_j\equiv q(\xi_j)\in\mathbb{R}^{n_2}$	& second-stage cost vector \\
			$h_j\equiv h(\xi_j)\in\mathbb{R}^{m_2}$	& second-stage RHS vector\\ \midrule
			%$\mathbf{0}\in\mathbb{R}^{rn_1}$	& vector filled with zeros \\ \midrule
			\textbf{Matrices} &  \\  \midrule
			$A\in\mathbb{R}^{m_1\times n_1}$	& first-stage constraint matrix corresponds to the decision vector $x$\\
			$W_j\equiv W(\xi_j)\in\mathbb{R}^{m_2\times n_2}$	& second-stage constraint matrix corresponds to the decision vector $y_j$\\
			$T_j\equiv T(\xi_j)\in\mathbb{R}^{m_2\times n_1}$	& second-stage constraint matrix corresponds to the decision vector $x_j$\\ \midrule
			%$H_j\equiv H(\xi_j)\in\mathbb{R}^{rm_1\times n_1}$	&	nonanticipativity constraints matrix \\ \midrule
			\textbf{Functions}	&	\\ \midrule
			$\phi_{\xi_j}:\mathbb{R}^{m_2}\to\mathbb{R}$	& second stage program optimal value given the realization of scenario $\xi_j$	\\
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{@{}llll@{}}
			\toprule
			Problem name 		  & Description                                                        & Reference              \\ \midrule
			\texttt{DCAP}         & Dynamic capacity planning with stochastic demand                   & Ahmed and Garcia                          \\
			\texttt{MPTSPs}       & Multi-path traveling salesman problem with stochastic travel costs & \cite{journal:MPT2014,journal:PGM2017,journal:TPP2017}                            \\
			\texttt{\texttt{SMKP}}& Stochastic multiple knapsack problem                               & Angulo et al.                            \\
			\texttt{SIZES}        & Optimal product substitution with stochastic demand                & Jorjani et al.                             \\
			\texttt{SSLP}         & Stochastic server location problem                                 & Ntaimo and Sen                            \\
			\texttt{WECC}         & Wind power stochastic unit commitment				               & Papavasiliou and Oren                       \\ \bottomrule
		\end{tabular}%
	}
\end{table}
