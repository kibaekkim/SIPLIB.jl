...
\begin{document}
	...
	\begin{abstract}
		...
		The test set includes SIP instances from eleven different problems arising in applications to power systems, telecommunications, manufacturing, and so on. In addition to a set of pre-generated \smps\ format instances, \siplibtwo\ provides 
		an open-source \julia\ package for modeling new SIP instances and writing them in \smps\ format, where 
		the stochastic programming problems can be modeled using the algebraic modeling package 
		...
		optimality gap, integrality gap, value of stochastic solutions). For the computational results, 
		we use an open-source parallel decomposition solver DSP together with a generic optimization solver 
