...
% abstract
\begin{abstract}
Advances in numerical methods for stochastic (mixed-) integer 
programming (SIP) has been made for the last decades. Many numerical 
...
test instances.  We present a set of test problem instances for SIP. In addition to the existing \siplib\ instances available in 
\url{https://www2.isye.gatech.edu/~sahmed/siplib/}, we provide {\color{red} XX} additional 
test instances arising in applications to energy, {\color{red} and so on}. \siplibtwo\ provides 
...
CPLEX. \kk{Do we want to try PySP?} \yoc{I think CPLEX and DSP are enough unless our purpose is on comparing the performance of each different solver. One more thing is, PySP does not read SMPS files.}
