...
\begin{document}
	...
	\begin{frame}{Limitation of the former \siplib}
		\begin{enumerate}
			...
			\end{itemize}
			\pause
			\invisible<-1>{\item Variation of problem types}
			\begin{itemize}
				\invisible<-1>{\item Current SIPLIB provides only 5 different variations in terms of variable types: continuous, binary, integer.}
				\invisible<-1>{\item 24 types exist in total for two-stage SIP.}
			\end{itemize}
			\pause		
			\invisible<-2>{\item Contribution rule}
			\begin{itemize}
				\invisible<-2>{\item Different problem provides different information.}
				\invisible<-2>{\item For some problems, only limited information is available.}
			\end{itemize}
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}{Contribution of \siplibtwo}
		...
		\pause
		\begin{itemize}
			\invisible<-1>{\item \textcolor{blue}{richer instances} accompanied with analytic \& computational information}
			\pause
			\invisible<-2>{\item \textcolor{blue}{a \julia\ package} to support computational research studies for SIP} 
			\begin{itemize}
			...
			\begin{itemize}
				...
				\invisible<-3>{\item parameterized modeling scripts}
			\end{itemize}
		\end{itemize}
	\end{frame}
	...
	\begin{itemize}
		...
		\pause
		\invisible<-3>{\item First-stage decision affects the second-stage decision as well.}
	\end{itemize}
	\end{frame}
	...
	\begin{frame}{EF: Block-diagonal structure}
		...
		\begin{figure}
			...
			\vspace{0.5cm}
			\caption{Example: Sparsity patterns in EF}
			%\label{data3}
		\end{figure}
	\end{frame}

	\section{\textbf{\texttt{SMPS}}, \textbf{\texttt{Julia}}, \textbf{\texttt{StructJuMP}}}
	\begin{frame}<beamer>{Contents}
		\tableofcontents[currentsection,currentsubsection,hideothersubsections]
	\end{frame}	
	...
	\begin{frame}{\texttt{SMPS} format}
		...
		\pause
		\begin{itemize}
			\invisible<-1>{\item \textbf{(example)} Let DCAP\_3\_3\_3\_10 be the instance name. Then, \smps\ format comprises}
			\begin{itemize}
				\invisible<-1>{\item DCAP\_3\_3\_3\_10.cor}
				\invisible<-1>{\item DCAP\_3\_3\_3\_10.tim}
				\invisible<-1>{\item DCAP\_3\_3\_3\_10.sto}
			\end{itemize}
			\pause
			\invisible<-2>{\item The role of each file:}
			\begin{itemize}
				\invisible<-2>{\item \textbf{\texttt{.cor}}: Core file written in \mps\ format. This describes the \textcolor{blue}{fundamental problem structure}.}
				\invisible<-2>{\item \textbf{\texttt{.tim}}: Time file which specifies \textcolor{blue}{the location where each stage begins.}}
				\invisible<-2>{\item \textbf{\texttt{.sto}}: Stoch file which contains \textcolor{blue}{remaining scenario data.}}
			\end{itemize}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{\structjump\ package}
		...
		\pause
		\begin{itemize}
			\invisible<-1>{\item \textbf{\structjump} is an \textcolor{blue}{extension of \textbf{\jump} package}, a domain-specific modeling language for mathematical optimization embedded in \julia.}
			\begin{itemize}
				\invisible<-1>{\item \textbf{\jump} is generic, fast, and straightforward modeler.}
			\end{itemize}
			\pause
			\invisible<-2>{\item \textbf{\structjump} constructs a \textcolor{blue}{\textbf{\jumpmodel}-type object} that contains every information of an SIP instance.}
		\end{itemize}
	\end{frame}
...
%	\end{frame}
	...
	\begin{frame}{Core functionality of \siplibjl}
		...
		\begin{figure}
			\begin{center}
				\invisible<-1>{\includegraphics[width=\textwidth]{siplib_package}}
			\end{center}
		\end{figure}	
	\end{frame}
...
%			\caption{Components of the problems}
	...
	\begin{frame}{Computing environment (in progress)}
		\begin{figure}
			\begin{center}
