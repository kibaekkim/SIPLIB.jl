...
for prob in problem
    ...
    for params_arr in param_set[prob]
        ...
        push!(tempv, sz.nRow)
        push!(tempv, sz.nCol)
        push!(tempv, round(sp.sparsity_A*100, 2))
        push!(tempv, round(sp.sparsity_W*100, 2))
        push!(tempv, round(sp.sparsity_T*100, 2))
        push!(tempv, round(sp.sparsity*100, 2))
        push!(tempv, round(filesize(SMPS_PATH*"/$(String(prob))/$(getInstanceName(prob, params_arr)).cor")/1000 , 2))
        push!(tempv, round(filesize(SMPS_PATH*"/$(String(prob))/$(getInstanceName(prob, params_arr)).tim")/1000 , 2))
        push!(tempv, round(filesize(SMPS_PATH*"/$(String(prob))/$(getInstanceName(prob, params_arr)).sto")/1000 , 2))
        print(tempv)
        push!(df,tempv)
    end
